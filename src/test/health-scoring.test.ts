...
suite('WinCC OA Project Health Assessment Tests', () => {
	...
	suite('Health Score Integration Tests', () => {
		...
		test('should handle multiple concurrent issues', () => {
			...
			assert.ok(health.recommendations.length > 3, `Expected multiple recommendations, got ${health.recommendations.length}`);
			
			// Should result in poor overall score
			assert.ok(health.overallScore < 60, `Expected poor score for problematic system, got ${health.overallScore}`);
