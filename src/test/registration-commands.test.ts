...
function createMockProject(config: Partial<MockProjectConfig>, isRunnable: boolean = true, isCurrent: boolean = false, version?: string): WinCCOAProject {
...
suite('Registration Commands Test Suite', () => {
    ...
    suite('Command Registration', () => {
        test('should have registerRunnableProject command available', async () => {
            ...
            assert.ok(commands.includes('winccOAProjects.registerRunnableProject'), 
                'registerRunnableProject command should be registered');
        });

        test('should have registerSubProject command available', async () => {
            const commands = await vscode.commands.getCommands(true);
            assert.ok(commands.includes('winccOAProjects.registerSubProject'), 
    ...
    suite('Project Registration Helpers', () => {
        ...
        test('should handle project already registered scenario', () => {
            ...
            const isAlreadyRegistered = existingProjects.some(p => 
                path.normalize(p.config.installationDir).toLowerCase() === normalizedPath
            );
...
`;
        ...
        test('should handle various version formats', () => {
            ...
            versionTests.forEach(test => {
                ...
                const extractedVersion = versionMatch ? versionMatch[1] : undefined;
                assert.strictEqual(extractedVersion, test.expected, 
                    `Version extraction failed for: ${test.content}`);
            });
        });
    });
        ...
        test('should validate directory vs file distinction', () => {
            ...
            testCases.forEach(testCase => {
                ...
                if (!testCase.isDirectory) {
                    ...
                    assert.ok(error.message.includes('directories'), 
